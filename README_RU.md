...
<p align="center"><strong>Текущий релиз: v0.3.1</strong> — версии для Windows и macOS собираются из одного тега и публикуются одновременно.</p>

<p align="center">
  <img alt="Текущая версия v0.3.1" src="https://img.shields.io/badge/current-v0.3.1-6f8f67?style=flat-square">
...
- **Осторожная классификация активности** прежде всего по активному приложению. Заголовок уточняет категорию только при явных признаках; неизвестная и смешанная работа так и обозначаются.
...
Daytrace использует нативные события активного окна и редкие замеры вместо скриншотов и постоянного сканирования экрана. На тестовом Windows-компьютере версия v0.3.1 показала **не более 0,04% общей фоновой нагрузки CPU** в 30-секундных замерах и **188 МиБ суммарной рабочей памяти** после выгрузки интерфейса. Открытие окна заняло **0,82 с** от старта процесса до появления интерфейса и **0,22 с** при быстром повторном открытии из трея. Результат зависит от компьютера, антивируса, количества событий и ОС; сборка macOS проверяется в CI, но сопоставимые замеры на физическом Mac ещё собираются.
