...
<p align="center"><strong>Текущий релиз: v0.5.6</strong> — версии для Windows и macOS собираются из одного тега и публикуются одновременно.</p>
...
<p align="center">
  <img alt="Текущая версия v0.5.6" src="https://img.shields.io/badge/current-v0.5.6-6f8f67?style=flat-square">
...
Daytrace не требует аккаунта или API-ключа. Сбор, фильтрация, группировка, ответы и удаление происходят на вашем компьютере. Сбор и анализ работают без интернета. Сеть используется только для двух явных операций с GitHub: проверяемых обновлений и опциональной отдельной загрузки пакета локального классификатора. Данные активности ни в один запрос не входят.
...
Для одного независимо отправленного Windows-артефакта доступен публичный [отчёт VirusTotal](https://www.virustotal.com/gui/file/9a898a7eb1bec9ee2d7b97d3593f7c17b02a9902dbbdb24959b1528082c117a6/detection) с SHA-256 `9a898a7eb1bec9ee2d7b97d3593f7c17b02a9902dbbdb24959b1528082c117a6`. Отчёт относится только к файлу с точно таким hash; это дополнительная проверка несколькими движками, а не замена `SHA256SUMS.txt`, просмотру исходников или новой проверке следующей сборки.
...
- **Опциональный умный локальный анализ**: небольшой RU/EN-пакет классификатора скачивается отдельно с проверкой SHA-256, запускается короткой задачей только во время простоя или вручную и выгружается из памяти после обработки. Базовый режим полностью работает без него.
- **Опциональное дополнение для Chromium** — Chrome, Edge, Brave и Vivaldi. Через native messaging оно передаёт только заголовок активной вкладки, домен, безопасный путь и приватный признак; параметры, фрагменты, логины, содержимое страницы и фоновые вкладки отбрасываются.
...
- **Встроенная самодиагностика** хранилища, сборщика, заголовков, idle-сигнала, приватного фильтра, Accessibility, автозапуска, браузерного дополнения и опциональной модели.
...
Изменения классификатора защищены версионированным синтетическим RU/EN-набором для браузеров, мессенджеров, IDE, игр, видео, документов, встреч и обучения. CI требует не менее 90% покрытия, не менее 94% точности среди классифицированных примеров, не менее 90% правильных ответов на каждом языке и ни одной принудительной метки для явно неоднозначных случаев. Это защита от регрессий, а не обещание понять любой реальный заголовок.
...
У Daytrace нет телеметрии и облачного анализа. Обновлятор обращается к официальному GitHub Releases только с номером установленной версии и сверяет артефакт с `SHA256SUMS.txt`. Если пользователь явно скачивает опциональный классификатор, Daytrace получает asset строго из релиза установленной версии и проверяет его как по release-checksum, так и по SHA-256, встроенному в эту версию приложения. События, заголовки, домены, вопросы, правила и настройки ни в один запрос не входят. Browser companion обменивается данными только через локальный пользовательский pipe/socket со случайным токеном.
...
Для локальных ответов LLM не используется. Детерминированный анализатор на устройстве распознаёт точные и относительные даты, сравниваемые периоды, время, приложение, цель и тип вопроса — сводку, длительность, последнюю активность, вкладки или переключения — и рассчитывает ответ по локальным сессиям. Опциональная модель лишь уточняет классификацию неоднозначных видимых заголовков: она не генерирует текст, не получает сырой журнал, не остаётся в памяти и не делает сетевые inference-запросы. Над ответом показывается, как именно был понят вопрос.
...
Browser companion работает от событий и не использует цикл опроса. Его native bridge запускается только для одной передачи контекста активной вкладки и сразу завершается, поэтому расширение не оставляет второй процесс Electron постоянно в памяти. Умный анализ по умолчанию выключен; после включения он ждёт не менее двух минут простоя, обрабатывает ограниченную пачку в отдельном worker и завершает процесс. Старые дни и годовая история загружаются лениво, а не анализируются постоянно.
...
- Локальные ответы детерминированы и основаны на правилах; опциональный классификатор не является разговорной LLM.
