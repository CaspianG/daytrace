...
После запуска приложение отдельно объяснит, зачем нужен «Универсальный доступ», и зарегистрирует именно помощник **Daytrace Collector**, который читает метаданные активного окна. Включите в списке **Daytrace Collector**: старые переключатели Daytrace или Daytrace 2 относятся к окну приложения и не разрешают работу сборщика. Daytrace проверяет доступ настоящим запуском этого же помощника и начинает запись без обычного перезапуска. Окно разрешения всегда можно закрыть — оно больше не блокирует настройки и уже сохранённую локальную историю. Оставьте одну `/Applications/Daytrace.app`; встроенное обновление само исправляет номерные дубликаты. Перед открытием DMG сверьте его с `SHA256SUMS.txt` из того же релиза.

![Закрываемая настройка разрешения Daytrace Collector на русском](docs/assets/screenshots/mac-permission-ru.png)
