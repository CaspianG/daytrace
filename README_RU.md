...
<p align="center"><strong>Текущий релиз: v0.5.13</strong> — версии для Windows и macOS собираются из одного тега и публикуются одновременно.</p>

> **Важно перед первым запуском на macOS:** текущая версия для Mac бесплатна и полностью локальна, но не подписана Apple Developer ID и не нотарифицирована, потому что у проекта нет этих учётных данных Apple. Поэтому Gatekeeper покажет предупреждение. Перед скачиванием откройте [безопасную инструкцию по установке на macOS](docs/MACOS_INSTALL_RU.md): она использует штатные пункты **«Открыть»** / **«Всё равно открыть»** и не отключает Gatekeeper.

<p align="center">
  <img alt="Текущая версия v0.5.13" src="https://img.shields.io/badge/current-v0.5.13-6f8f67?style=flat-square">
...
После запуска приложение отдельно объяснит, зачем нужен «Универсальный доступ», и зарегистрирует именно помощник **Daytrace Activity Collector**, который читает метаданные активного окна. Включите **Daytrace Activity Collector**: старые записи Daytrace, Daytrace 2 или Daytrace Collector не разрешают работу текущего помощника. В v0.5.13 исправлен постоянный ложный отказ: проверка разрешения и настоящий сбор теперь запускают один и тот же вложенный `.app` одним способом через macOS LaunchServices. Сборщик сам подтверждает готовность по защищённому локальному каналу — Daytrace больше не доверяет разрешению интерфейсного процесса или одному нажатию кнопки. Если macOS сохранила устаревшую запись, кнопка **«Восстановить разрешение Daytrace»** сбросит только разрешение этого сборщика и сразу зарегистрирует его заново; разрешения других программ не затрагиваются. Фиксированная bundle-идентичность и одна защищённая самоподписанная community-идентичность сохраняются между релизами. Community-подпись не убирает предупреждение Gatekeeper и не заменяет нотарификацию Apple. Окно разрешения всегда можно закрыть — оно не блокирует настройки и уже сохранённую историю. Оставьте одну `/Applications/Daytrace.app`; встроенное обновление само исправляет номерные дубликаты. Перед открытием DMG сверьте его с `SHA256SUMS.txt` из того же релиза.

![Закрываемая настройка разрешения Daytrace Activity Collector на русском](docs/assets/screenshots/mac-permission-ru.png)
...
- **Раскрываемое распределение целей**: нажмите на «Работа», «Обучение», «Личное», «Развлечения», «Смешанная» или «Неоднозначная цель», чтобы увидеть точные локальные интервалы от новых к старым — приложение, наблюдаемый контекст активного окна, время, длительность и уверенность определения. Открытие деталей не запускает модель и не сканирует архив заново.
...
- **Стабильная работа с длинной историей**: выход из «Обзора» больше не выбрасывает выбранный день из памяти, фоновые heartbeat-события не могут на мгновение подставить чужой 48-часовой контекст, а большие сессии раскрываются небольшими порциями вместо одновременного создания тысяч строк.
...
- **Системная, светлая и тёмная темы**, которые хранятся локально: нативная рамка окна использует тот же режим, в боковой панели есть быстрое переключение, в настройках — полный выбор, а анимация охватывает всё приложение и учитывает уменьшение движения.
- **Выбор языка при первом запуске** и мгновенное переключение позже в настройках.
- **Пошаговая настройка разрешения macOS** для точного вложенного процесса **Daytrace Activity Collector**, с прямой ссылкой в системные настройки и проверкой от его собственной стабильной TCC-идентичности. После обновления с v0.5.11 или старее новый сборщик нужно включить один раз; первое предупреждение Gatekeeper останется до появления Apple Developer ID и нотарификации.
...
- **Три режима локального анализа**: «Встроенный» ничего не скачивает; «Пакет сигналов 1.1» — прозрачный RU/EN-файл весов слов и фраз размером около 6 КБ с проверкой SHA-256; «Семантическая модель 1.0» — опциональная связка RU+EN-энкодеров размером около 48 МБ, которая сравнивает смысл, а не только точные ключевые слова. Выбранный дополнительный режим запускается ограниченным коротким воркером после пяти минут системного простоя при питании от сети или вручную. Семантический анализ использует один поток процессора и выгружает воркер одного языка до запуска следующего. Это не чат и не генеративная LLM; Daytrace полностью работает без модели.
- **Честное сравнение качества** при первом запуске и в настройках. Для каждого режима показаны точность решений и покрытие на одном версионном RU/EN-наборе. Отдельный локальный проход считает, какую долю сохранённой истории определяет каждый установленный режим, и сравнивает исходные прогнозы только с теми контекстами, которые пользователь явно исправил. Заголовки не попадают в агрегированный файл качества, а малая личная выборка помечается как предварительная, а не выдаётся за надёжную точность.
- **Первичная настройка, которая действительно запускается один раз для локального профиля**, объясняет приватность и предлагает новому пользователю явно выбрать встроенный режим, пакет сигналов либо семантическую модель. Обновления её не открывают. Отдельная анимированная подсказка из пяти шагов один раз проходит по настоящему интерфейсу и затем остаётся доступной из боковой панели и настроек, не меняя выбранные параметры.
...
![Настройки тёмной темы Daytrace на русском](docs/assets/screenshots/appearance-dark-ru.png)
...
## Одна короткая настройка и подсказка — дальше Daytrace учится локально

Версия 0.5.9 разделяет обязательную первичную настройку и помощь по интерфейсу. Настройка открывается только у локального профиля, который ещё ни разу её не завершал; смена версии приложения больше не может открыть этот экран заново. После настоящего первого запуска пользователь один раз видит короткую анимированную подсказку прямо поверх обзора, строки вопроса, исправления цели, выбора модели и локального статуса. Факт показа сохраняется сразу при открытии, поэтому аварийное закрытие не создаёт бесконечный цикл. Повторить подсказку всегда можно вручную из боковой панели или настроек без сброса параметров.

Встроенный режим остаётся выбранным по умолчанию ради нулевой загрузки и минимальной нагрузки. Семантический режим покрывает больше общей контрольной проверки, но остаётся опциональным, проверяется по SHA-256, использует один поток, автоматически ждёт простоя и питания от сети, а после короткого пакета выгружается. Две более крупные многоязычные модели были воспроизводимо проверены и отклонены: они оказались примерно в 2,7 раза тяжелее и заметно хуже по точности решений на отложенной проверке. Daytrace не добавляет тяжёлую модель только ради красивого названия.
...
![Повторяемая подсказка из пяти шагов поверх настоящего интерфейса Daytrace на русском](docs/assets/screenshots/quick-tour-ru.png)

![Уведомление Daytrace о разборе низкой уверенности на русском](docs/assets/screenshots/review-coach-ru.png)

![Сравнение трёх режимов и локальная проверка качества по личной истории на русском](docs/assets/screenshots/analysis-quality-ru.png)

| Режим | Точность решений | Покрытие | Загрузка / компромисс |
| --- | ---: | ---: | --- |
| Встроенный | около 93% (25/27) | 56% (27/48) | 0 МБ, мгновенно и детерминированно |
| Пакет сигналов 1.1 | около 93% (26/28) | 58% (28/48) | около 6 КБ, прозрачные веса фраз |
| Семантическая модель 1.0 | около 95% (35/37) | 77% (37/48) | около 48 МБ, короткие пакеты в один поток |

Все цифры получены на одних и тех же 48 размеченных RU/EN-случаях. «Точность решений» не учитывает пропуски, а «покрытие» показывает, как часто режиму хватило признаков для ответа. Это не универсальная точность на личной истории. Поэтому в настройках есть второе, полностью локальное сравнение: покрытие по уникальным контекстам сохранённой истории и совпадение только с явными исправлениями пользователя, с точным размером выборки и предупреждением при менее чем 15 исправленных контекстах. Установленному опциональному режиму не приписывается скопированная или расчётная личная цифра до первого настоящего локального прохода — интерфейс честно просит один раз запустить анализ.
...
Изменения классификатора защищены версионированными синтетическими RU/EN-наборами для браузеров, мессенджеров, IDE, игр, видео, документов, встреч и обучения. Общее сравнение выше заново считается из исходников командой `npm run model:verify:semantic`; CI падает, если зафиксированные числа расходятся с результатом. На отложенной части набора встроенный режим и пакет сигналов сейчас дают примерно 91% точности решений при 72% покрытия, а семантический — примерно 91% точности при 69% покрытия. Реальная история содержит гораздо более короткие и шумные метаданные, поэтому слабые заголовки, имена чатов, ценовые тикеры, общие окна почты/поиска и обрезанный текст интерфейса отбрасываются до запуска модели. Проверка на устройстве потоково читает сохранённые JSONL в фоновом воркере и записывает только счётчики и отношения. Это диагностика регрессий, а не обещание понять любой реальный заголовок.
...
Daytrace использует нативные события активного окна и редкие замеры вместо скриншотов и постоянного сканирования экрана. В Windows-замере v0.5.10 фоновый Electron без окна интерфейса в среднем использовал **0,032% CPU** по выборке Electron-процессов за 12 секунд; основной процесс за тот же интервал показал **0,392% CPU**. Пик составил **125,6 МиБ private memory / 193,9 МиБ working set**, рост памяти за замер — только **0,1 МиБ**, служебных Electron-процессов было три. Финальный нативный Windows-сборщик измерен отдельно в течение 20,10 секунды: за этот интервал он не набрал измеримого CPU-времени, использовал максимум **28,6 МиБ working set / 7,0 МиБ private memory** и 13 потоков. Финальная проверка Windows-пакета v0.5.11 показала **0,023% среднего CPU Electron**, **84,9 МиБ peak private memory** и **0,1 МиБ** роста за короткий замер. Предрелизная фоновая проверка v0.5.12 после добавления расшифровки целей показала **0,000% CPU основного процесса**, **0,013% среднего CPU Electron**, **84,5 МиБ peak private memory** и **0,1 МиБ** роста за 12 секунд. Windows-замер v0.5.13 показал **0,125% CPU основного процесса**, **0,010% среднего CPU Electron**, **83,8 МиБ peak private memory** и **0,0 МиБ** кратковременного роста private memory за 12 секунд. Это замеры development-runtime, а не одинаковая гарантия для любого компьютера. Автоматическая проверка релиза отклоняет сборку, если средняя фоновая нагрузка Electron превышает **5% CPU** или **180 МиБ private memory**. На сохранённой локальной истории, использованной для того замера, холодная подготовка состояния за последние 48 часов заняла около **499 мс**, повторное открытие выбранного дня — около **0,01 мс**, а пересчёт после нового события — около **220 мс**. В v0.5.11 отдельная проверка навигации по плотной истории уменьшила состояние, отправляемое в интерфейс, примерно с **2,3 МБ до 57 КБ**; в проверенном изолированном прогоне с 900 контекстами «Обзор» повторно открылся за **18 мс**, самая длинная задача renderer заняла **116 мс**, а одновременно было создано только **24 строки активности**. Расшифровка v0.5.12 читает только уже загруженный выбранный день и раскрывает длинные списки порциями по 12 строк; она не запускает модель и не перечитывает архив. Заголовки устройство не покидали. Обновления делают один небольшой запрос метаданных после запуска и затем не чаще раза в шесть часов; постоянного сетевого опроса нет. Сопоставимые замеры на физическом Mac ещё собираются и не подменяются цифрами Windows.

Стабильность проверяется отдельно от обычных unit-тестов. Desktop-smoke намеренно завершает Windows-сборщик и принудительно аварийно останавливает Electron-renderer, после чего требует автоматического восстановления обоих компонентов. Перезапуск использует ограниченный экспоненциальный backoff, десятисекундный watchdog готовности сборщика и предел в три попытки для renderer, поэтому неисправный компонент не превращается в бесконечный быстрый цикл. При сне или блокировке компьютера сборщик останавливается, а после пробуждения или разблокировки запускается заново и восстанавливает системные hooks. В macOS проверки отсутствующего Accessibility-разрешения замедляются с двух до тридцати секунд вместо постоянного запуска helper-процессов, чтение Accessibility прекращается во время простоя, а отключённый event tap автоматически включается снова. У опционального browser companion есть такой же ограниченный путь восстановления.

Browser companion работает от событий и не использует цикл опроса. Его native bridge запускается только для одной передачи контекста активной вкладки и сразу завершается, поэтому расширение не оставляет второй процесс Electron постоянно в памяти. По умолчанию выбран встроенный режим без загрузки модели. Автоматический сигнальный или семантический проход ждёт не менее пяти минут простоя, питание от сети и новые контексты, обрабатывает ограниченную пачку и завершает worker; ручной запуск доступен в любое время. В текущем Windows-smoke-тесте Electron/WASM двуязычный семантический проход проверил два неоднозначных контекста за **2,19 с**: приватная память всего фонового дерева процессов временно выросла с **149,2 до 561,1 МиБ**, после чего скрытый процесс анализа был уничтожен. Это намеренно консервативный тест RU+EN, а не постоянная фоновая нагрузка; проход для одного языка может быть легче, а результат зависит от компьютера. Старые дни и годовая история загружаются лениво, а не анализируются постоянно.
