...
<p align="center"><strong>Текущий релиз: v0.5.10</strong> — версии для Windows и macOS собираются из одного тега и публикуются одновременно.</p>
...
<p align="center">
  <img alt="Текущая версия v0.5.10" src="https://img.shields.io/badge/current-v0.5.10-6f8f67?style=flat-square">
...
Daytrace использует нативные события активного окна и редкие замеры вместо скриншотов и постоянного сканирования экрана. В свежем Windows-замере v0.5.10 фоновый Electron без окна интерфейса в среднем использовал **0,032% CPU** по выборке Electron-процессов за 12 секунд; основной процесс за тот же интервал показал **0,392% CPU**. Пик составил **125,6 МиБ private memory / 193,9 МиБ working set**, рост памяти за замер — только **0,1 МиБ**, служебных Electron-процессов было три. Финальный нативный Windows-сборщик измерен отдельно в течение 20,10 секунды: за этот интервал он не набрал измеримого CPU-времени, использовал максимум **28,6 МиБ working set / 7,0 МиБ private memory** и 13 потоков. Это замеры development-runtime, а не одинаковая гарантия для любого компьютера. Автоматическая проверка релиза отклоняет сборку, если средняя фоновая нагрузка Electron превышает **5% CPU** или **180 МиБ private memory**. На сохранённой локальной истории, использованной для проверки этого релиза, холодная подготовка состояния за последние 48 часов заняла около **499 мс**, повторное открытие выбранного дня — около **0,01 мс**, а пересчёт после нового события — около **220 мс**; заголовки устройство не покидали. Обновления делают один небольшой запрос метаданных после запуска и затем не чаще раза в шесть часов; постоянного сетевого опроса нет. Сопоставимые замеры на физическом Mac ещё собираются и не подменяются цифрами Windows.
