...
<p align="center"><strong>Текущий релиз: v0.5.12</strong> — версии для Windows и macOS собираются из одного тега и публикуются одновременно.</p>
...
<p align="center">
  <img alt="Текущая версия v0.5.12" src="https://img.shields.io/badge/current-v0.5.12-6f8f67?style=flat-square">
...
После запуска приложение отдельно объяснит, зачем нужен «Универсальный доступ», и зарегистрирует именно помощник **Daytrace Activity Collector**, который читает метаданные активного окна. Включите **Daytrace Activity Collector**: старые записи Daytrace, Daytrace 2 или Daytrace Collector не разрешают работу помощника v0.5.12. Это одноразовый переход на фиксированную bundle-идентичность и одну защищённую самоподписанную community-идентичность в следующих релизах — раньше TCC-идентичность ошибочно менялась вместе с каждой версией Daytrace. Теперь помощник запускается через macOS LaunchServices, Daytrace проверяет его собственный результат и явно показывает регистрацию, отказ, отсутствие файла или ошибку, а не считает успехом само нажатие кнопки. Community-подпись не убирает предупреждение Gatekeeper и не заменяет нотарификацию Apple. Окно разрешения всегда можно закрыть — оно не блокирует настройки и уже сохранённую историю. Оставьте одну `/Applications/Daytrace.app`; встроенное обновление само исправляет номерные дубликаты. Перед открытием DMG сверьте его с `SHA256SUMS.txt` из того же релиза.
