...
<p align="center"><strong>Текущий релиз: v0.5.1</strong> — версии для Windows и macOS собираются из одного тега и публикуются одновременно.</p>

<p align="center">
  <img alt="Текущая версия v0.5.1" src="https://img.shields.io/badge/current-v0.5.1-6f8f67?style=flat-square">
...
На macOS 12 или новее скачайте универсальный `Daytrace-…-macOS-universal.dmg` и перенесите Daytrace в «Программы». При первом запуске приложение объясняет, зачем нужен «Универсальный доступ», открывает точный раздел «Конфиденциальность и безопасность» и автоматически запускает сбор после включения Daytrace и возврата в приложение. Можно продолжить без разрешения, но история останется пустой, пока доступ не будет выдан.

Текущая сборка v0.5.1 для macOS ещё не notarized, поэтому Gatekeeper может потребовать выбрать **«Открыть»** в контекстном меню Finder. Релизный процесс теперь не позволит опубликовать следующую неподписанную или не прошедшую notarization сборку для macOS; чтобы убрать предупреждение в публичном релизе, владелец репозитория должен добавить сертификат Apple Developer ID и ключ notarization по [инструкции для macOS](docs/MACOS_SIGNING.md).
