...
После запуска приложение отдельно объяснит, зачем нужен «Универсальный доступ», откроет точный раздел «Конфиденциальность и безопасность» и автоматически запустит сбор после включения Daytrace и возврата в приложение. Можно продолжить без разрешения, но история останется пустой, пока доступ не будет выдан. Перед открытием DMG сверьте его с `SHA256SUMS.txt` из того же релиза.
