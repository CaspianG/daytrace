...
<p align="center"><strong>Текущий релиз: v0.5.3</strong> — версии для Windows и macOS собираются из одного тега и публикуются одновременно.</p>
...
<p align="center">
  <img alt="Текущая версия v0.5.3" src="https://img.shields.io/badge/current-v0.5.3-6f8f67?style=flat-square">
...
После запуска приложение отдельно объяснит, зачем нужен «Универсальный доступ», и откроет точный раздел «Конфиденциальность и безопасность». Оставьте только одну установленную копию `/Applications/Daytrace.app`: если при обновлении Finder предлагает «Заменить» или «Сохранить обе», выбирайте **«Заменить»**. Дубликат вроде `Daytrace 2.app` получает в macOS отдельную запись разрешения. Daytrace определяет запущенную копию, показывает её путь и предлагает перезапуск после выдачи доступа. Можно продолжить без разрешения, но история останется пустой, пока доступ не будет выдан. Перед открытием DMG сверьте его с `SHA256SUMS.txt` из того же релиза.
