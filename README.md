...
<p align="center"><strong>Current release: v0.5.12</strong> — Windows and macOS artifacts are built from the same tag and published together.</p>
...
<p align="center">
  <img alt="Current version v0.5.12" src="https://img.shields.io/badge/current-v0.5.12-6f8f67?style=flat-square">
...
After the app opens, Daytrace separately explains why Accessibility access is needed and registers the exact **Daytrace Activity Collector** helper that reads active-window metadata. In Accessibility, enable **Daytrace Activity Collector** — an old Daytrace, Daytrace 2, or Daytrace Collector switch does not authorize the v0.5.12 helper. This one-time migration gives the collector a fixed bundle identity and the same protected self-signed community identity in later releases, instead of changing its TCC identity with every Daytrace version. Daytrace launches the helper through macOS LaunchServices, verifies the helper's own result, and now shows whether registration is running, denied, unavailable, or failed rather than silently accepting a button press. This community signature does not remove Gatekeeper warnings or replace Apple notarization. The permission dialog remains dismissible and never locks settings or existing history. Keep one installed `/Applications/Daytrace.app`; the built-in updater repairs numbered duplicates automatically. Verify the DMG against the release's `SHA256SUMS.txt` before opening it.
