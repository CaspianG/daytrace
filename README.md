...
<p align="center"><strong>Current release: v0.5.13</strong> — Windows and macOS artifacts are built from the same tag and published together.</p>

> **macOS first-launch notice:** the current Mac build is free and fully local, but it is not signed with an Apple Developer ID or notarized because the project does not have those Apple credentials. Gatekeeper will therefore warn on first launch. Read the [safe macOS installation guide](docs/MACOS_INSTALL.md) before downloading; it uses Finder's supported **Open** / **Open Anyway** flow and does not disable Gatekeeper.

<p align="center">
  <img alt="Current version v0.5.13" src="https://img.shields.io/badge/current-v0.5.13-6f8f67?style=flat-square">
...
After the app opens, Daytrace separately explains why Accessibility access is needed and registers the exact **Daytrace Activity Collector** helper that reads active-window metadata. In Accessibility, enable **Daytrace Activity Collector** — old Daytrace, Daytrace 2, or Daytrace Collector entries do not authorize the current helper. v0.5.13 fixes the recurring false-denial loop by using the same nested helper app and the same LaunchServices path for permission checks and real collection. The helper reports its own authenticated readiness instead of Daytrace trusting the UI process or a successful button press. If macOS has retained a stale record, **Repair Daytrace permission** resets only this helper and immediately re-registers it; no other application's permissions are changed. The helper keeps its fixed bundle identity and protected self-signed community identity across releases. This community signature does not remove Gatekeeper warnings or replace Apple notarization. The permission dialog remains dismissible and never locks settings or existing history. Keep one installed `/Applications/Daytrace.app`; the built-in updater repairs numbered duplicates automatically. Verify the DMG against the release's `SHA256SUMS.txt` before opening it.

![Dismissible Daytrace Activity Collector permission setup in English](docs/assets/screenshots/mac-permission-en.png)
...
- **Click-through purpose breakdowns**: select Work, Learning, Personal, Entertainment, Mixed, or Ambiguous purpose to see the exact newest-first local intervals behind that total, including application, observed active-window context, time, duration, and inference confidence. Opening a breakdown never reruns a model or scans the archive.
...
- **Stable long-history navigation**: leaving Overview no longer discards the selected day, background heartbeats cannot temporarily substitute a different 48-hour context, and large sessions reveal activity in small batches instead of mounting thousands of rows at once.
...
- **Guided macOS permission setup** for the exact nested **Daytrace Activity Collector**, with a direct System Settings link and verification performed by that collector's own stable TCC identity. Users updating from v0.5.11 or earlier enable the newly named helper once; the first-launch Gatekeeper warning remains until Apple Developer ID notarization is available.
...
Daytrace uses native foreground events and coarse samples instead of screenshots or continuous screen polling. In the v0.5.10 Windows verification run, the renderer-free Electron background runtime averaged **0.032% CPU** across sampled Electron processes over 12 seconds; the main process measured **0.392% CPU** over the same interval. It peaked at **125.6 MiB private memory / 193.9 MiB working set**, grew by only **0.1 MiB** across the sample, and kept three Electron service processes. The final native Windows collector was measured separately over 20.10 seconds: it consumed no measurable CPU tick in that sample, peaked at **28.6 MiB working set / 7.0 MiB private memory**, and stayed at 13 threads. The final v0.5.11 Windows package gate measured **0.023% average sampled Electron CPU**, **84.9 MiB peak private memory**, and **0.1 MiB** short-run growth. The v0.5.12 pre-release background gate measured **0.000% main-process CPU**, **0.013% average sampled Electron CPU**, **84.5 MiB peak private memory**, and **0.1 MiB** short-run growth over 12 seconds after the purpose drill-down was added. The v0.5.13 Windows background sample measured **0.125% main-process CPU**, **0.010% average sampled Electron CPU**, **83.8 MiB peak private memory**, and **0.0 MiB** short-run private-memory growth over 12 seconds. These are development-runtime measurements rather than a hardware-independent guarantee. The automated release gate rejects an Electron background average above **5% CPU** or **180 MiB private memory**. On the retained local history used for that check, a cold 48-hour dashboard state took about **499 ms**, a cached selected day about **0.01 ms**, and a state refresh after a new event about **220 ms**. The v0.5.11 dense-navigation gate additionally reduced the state sent to the renderer from about **2.3 MB to 57 KB** on the retained verification profile; a validated isolated 900-context run reopened Overview in **18 ms**, limited the longest observed renderer task to **116 ms**, and mounted only **24 activity rows**. The v0.5.12 drill-down reads only that selected-day memory and reveals long lists in 12-row batches; it does not invoke a model or rescan retained files. No titles left the device. The updater adds one small metadata request after startup and then at most once every six hours while online; it does not continuously poll. Equivalent physical-Mac measurements are still being collected and are not inferred from the Windows result.
