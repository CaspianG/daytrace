...
export const translations = {
  en: {
    ...
    intent: {
      ...
      reasons: { "custom-rule": "Your local rule for this app or exact context", "smart-model": "Optional local smart model; processed only safe stored metadata", "window-title": "Meaning of the active title", service: "Recognized active service", "application-category": "Specialized application category", "sequence-context": "Matching automatic evidence on both sides", "session-context": "Several matching automatic signals in this work block", "repeated-context": "Purpose learned from this repeated local context", "best-effort-work-app": "Low-confidence prior for a work tool", "best-effort-research": "Low-confidence research prior", "best-effort-messaging": "Best local estimate for an opaque chat", "best-effort-browser": "Best local estimate for an opaque browser page", "best-effort-application": "Best local estimate from the application type", "needs-context": "General-purpose app needs more context", "conflicting-title-signals": "Conflicting signals", "insufficient-evidence": "Not enough evidence", insufficient: "Not enough evidence" },
    },
    summary: {
    ...
    ask: {
      ...
      understood: "Understood as:", engineNote: "The base mode uses no LLM: a local parser handles dates, comparisons, applications, purposes, and question types. If the optional smart model is enabled, it runs only on safe stored metadata in a short-lived local worker.", demoInterpretation: "summary · today morning",
    ...
    settings: {
      ...
      smartAnalysis: "Optional local smart analysis", smartTitle: "Small local smart model", smartText: "Improves ambiguous RU/EN titles in batches while the computer is idle, then unloads from memory.", smartInstalled: "Model {version} is installed locally", smartNotInstalled: "The model is not installed", smartRunning: "Analyzing locally…", smartDownload: "Download model", smartFile: "Install model file", smartRun: "Analyze now", smartRemove: "Remove model", smartPrivacy: "The download request contains no activity data. Inference receives only the same safe app, title, and domain metadata already stored by Daytrace.",
      ...
      system: "System", autostart: "Launch at login", autostartText: "Starts quietly in the tray and begins local tracking", autostartUnavailable: "Available in the installed desktop application", runtimeText: "Native collector runs separately and writes only local events", accessibility: "Accessibility permission required", accessibilityText: "This is the only required macOS permission. Turn on Daytrace, then restart Daytrace if macOS still shows this message.", grantAccess: "Open permission settings", restartAfterAccess: "Restart Daytrace",
      diagnosticsTitle: "Self-diagnostics", diagnosticsText: "Checks the collector, local storage, title signal, idle boundaries, private filtering, autostart, browser companion, and smart model.", diagnosticsRun: "Run checks", diagnosticStatuses: { pass: "Works", warn: "Needs observation", fail: "Needs attention", "not-applicable": "Not applicable" }, diagnosticChecks: { storage: "Local data is writable", tracker: "Native collector", collector: "Collector executable", accessibility: "macOS Accessibility", titles: "Active titles arrive", idle: "Away detection", private: "Private windows are rejected", autostart: "Launch at login", browser: "Browser companion", smart: "Smart model" },
    ...
    onboarding: {
      ...
      permissionStepOne: "Click Allow access below.", permissionStepTwo: "Turn on the exact Daytrace copy in Privacy & Security → Accessibility.", permissionStepThree: "Return and click Restart Daytrace if macOS does not apply the permission immediately.",
      permissionCopyTitle: "macOS sees more than one copy", permissionPrivacy: "Daytrace reads only safe active-window metadata. It never records keys, typed text, screenshots, audio, or clipboard contents.", permissionGrant: "Allow access", permissionWaiting: "Opening macOS settings…", permissionRestart: "Restart Daytrace after enabling access", permissionLater: "Continue without tracking",
    },
    demo: {
  ...
  ru: {
    ...
    ask: {
      ...
      understood: "Понял вопрос как:", engineNote: "Базовый режим работает без LLM: локальный разборщик понимает даты, сравнения, приложения, цели и тип вопроса. Опциональная умная модель запускается кратким локальным процессом только на безопасных сохранённых метаданных.", demoInterpretation: "сводка · сегодня утром",
    ...
    settings: {
      ...
      smartAnalysis: "Опциональный умный локальный анализ", smartTitle: "Небольшая локальная модель", smartText: "Уточняет неоднозначные заголовки на русском и английском пакетно во время простоя, затем выгружается из памяти.", smartInstalled: "Модель {version} установлена локально", smartNotInstalled: "Модель не установлена", smartRunning: "Анализирую локально…", smartDownload: "Скачать модель", smartFile: "Установить файл модели", smartRun: "Проанализировать сейчас", smartRemove: "Удалить модель", smartPrivacy: "При скачивании данные активности не отправляются. Модель получает только те же безопасные названия приложения, окна и домена, которые уже сохранены Daytrace.",
    ...
    onboarding: {
      ...
      permissionStepOne: "Нажмите «Разрешить доступ» ниже.", permissionStepTwo: "Включите именно запущенную копию Daytrace в разделе «Конфиденциальность и безопасность» → «Универсальный доступ».", permissionStepThree: "Вернитесь и нажмите «Перезапустить Daytrace», если macOS не применила разрешение сразу.",
      permissionCopyTitle: "macOS видит несколько копий", permissionPrivacy: "Daytrace читает только безопасные метаданные активного окна. Нажатия клавиш, введённый текст, скриншоты, аудио и буфер обмена не записываются.", permissionGrant: "Разрешить доступ", permissionWaiting: "Открываю настройки macOS…", permissionRestart: "Перезапустить Daytrace после разрешения", permissionLater: "Продолжить без сбора",
    },
    demo: {
